...
export const episodeProgressKeys = {
  detail: (userId: string, mediaId: number) =>
    ['user-episode-progress', userId, mediaId] as const,
...
export const useEpisodeProgress = (userId: string, mediaId: number, enabled: boolean) => {
  const queryClient = useQueryClient()
  const queryKey = episodeProgressKeys.detail(userId, mediaId)

  const query = useQuery({
    queryKey,
    queryFn: () => getEpisodeProgress(userId, mediaId),
  ...
  return useMutation<void, Error, number>({
    ...
    onSuccess: async (_data, mediaId) => {
      await queryClient.invalidateQueries({
        queryKey: episodeProgressKeys.detail(userId, mediaId),
      })
    },
